...
// ${log.requestBody?.substring(0, 200)}...`;
  ...
  private static generateDetailedAnalysis(logs: NetworkLog[], payloadIssues: PayloadIssue[], timeoutIssues: NetworkLog[]): string {
    ...
    const actualTimeoutErrors = timeoutIssues.filter(log => 
      log.statusCode === 408 || 
      log.statusCode === 504 || 
  ...
  private static analyzeDataContent(successfulRequests: NetworkLog[]): string[] {
    ...
    const emptyResponses = successfulRequests.filter(log => {
      if (!log.responseBody) return true;
      try {
        const data = JSON.parse(log.responseBody);
  ...
  private static generateDataAnalysisSummary(successfulRequests: NetworkLog[]): string {
    if (successfulRequests.length === 0) {
    ...
    const jsonResponses = successfulRequests.filter(log => {
      if (!log.responseBody) return false;
      try {
    ...
    jsonResponses.forEach(log => {
      try {
        ...
        // Check for error flags in successful responses
        if (typeof data === 'object' && (data.error || data.errors || data.success === false)) {
          dataPatterns.push(`Error flag in successful response: ${log.url}`);2
        }
    ...
    const businessLogicAPIs = successfulRequests.filter(log => 
      log.url.includes('permissions') || 
