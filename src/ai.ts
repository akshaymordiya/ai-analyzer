import { JiraIssue, NetworkLog, AnalysisResult, MSHTicket } from './types.js';
import chalk from 'chalk';
import { getGroqKey } from './config.js';
import { DetailedNetworkAnalysis, PayloadIssue } from './network-analyzer.js';
...
class GroqClient {
  ...
  public parseJsonFromResponse(content: string): any {
  ...
  private extractJsonContent(content: string): string | null {
  ...
  private parseWithStrategy1(jsonContent: string): any {
    // Strategy 1: Direct JSON parse
    return JSON.parse(jsonContent);
  }

  private parseWithStrategy2(jsonContent: string): any {
  ...
  private parseWithStrategy3(jsonContent: string): any {
  ...
  private parseWithStrategy4(jsonContent: string): any {
  ...
  private parseWithStrategy5(jsonContent: string): any {
  ...
  private normalizeJiraComment(result: any): any {
...
export async function analyzeMSHTicket(prompt: string): Promise<any> {
...
function generateDetailedErrorAnalysis(logs: NetworkLog[]): string {
  if (logs.length === 0) return 'No network logs available';
  
  const failedRequests = logs.filter(log => log.statusCode >= 400);
  if (failedRequests.length === 0) return 'No failed requests found';
...
function generateFailedRequestsDetails(logs: NetworkLog[]): string {
  const failedRequests = logs.filter(log => log.statusCode >= 400);
  if (failedRequests.length === 0) return 'No failed requests found';
...
export async function analyzeWithAI(issue: string, logs: string, diff: string): Promise<string> {
